        ...
        features: [
          {
            ...
            geometry: {
              type: "LineString",
              coordinates: getPointCoordinates(from, destination, stopovers),
            },
          },
        ],
      });
  ...
  return (
    <div ref={mapContainerRef} id="map">
      ...
      >
        {from && (
          <Marker
            longitude={from._geoloc.lng}
            latitude={from._geoloc.lat}
            anchor="bottom"
          >
        ...
        {stopovers &&
          stopovers.map((stop) => {
            return (
              <Marker
                key={stop.iata_code}
                longitude={stop._geoloc.lng}
                latitude={stop._geoloc.lat}
                anchor="center"
              ></Marker>
            );
          })}
        {destination && (
          <Marker
            longitude={destination._geoloc.lng}
            latitude={destination._geoloc.lat}
            anchor="bottom"
          >
            ...
            >
              {destination.city} ({destination.iata_code}) destination
            </Popup>
          </Marker>
        )}
        {from && destination && (
...
const getMinOrMax = (
  ...
  if (minOrMax === "min") {
    return minBy(markers, (marker) => marker._geoloc[latOrLng])["_geoloc"][
      latOrLng
    ];
  } else
    return maxBy(markers, (marker) => marker._geoloc[latOrLng])["_geoloc"][
      latOrLng
    ];
};
...
const getPointCoordinates = (from, destination, stopovers) => {
  const stopoverCoords =
    stopovers?.map((stop) => [stop._geoloc.lng, stop._geoloc.lat]) || [];
  console.log(stopoverCoords);
  return [
    [from?._geoloc.lng, from?._geoloc.lat],
    ...stopoverCoords,
    [destination?._geoloc.lng, destination?._geoloc.lat],
  ];
};
